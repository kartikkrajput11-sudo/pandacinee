...
function PoolPage() {
  ...
  return (
    <div className="min-h-screen bg-velvet text-candle relative overflow-hidden">
      {matchId && <GroupPlayersBar partnerId={data?.partner?.id ?? null} matchId={matchId} meId={me?.id} gameName="8-Ball Pool" />}
  ...
  return (
    <div className="shrink-0 flex flex-col items-center gap-2 sm:gap-3 select-none">
      ...
      >
        ...
        />
        {[0.25, 0.5, 0.75].map((f) => (
          <div key={f} className="absolute left-1 right-1 h-px bg-white/25" style={{ bottom: `${f * 100}%` }} />
        ))}
        <div
